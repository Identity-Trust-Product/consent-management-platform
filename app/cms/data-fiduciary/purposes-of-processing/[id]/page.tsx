...
import { PurposeOfProcessingView } from "@/app/cms/data-fiduciary/purposes-of-processing/_components/purpose-of-processing-view";

interface PageProps {
  params: Promise<{ id: string }>;
}

async function PurposeOfProcessingViewContent({ params }: PageProps) {
  try {
    await verifyPermission(Resources.PURPOSE_OF_PROCESSING, Actions.READ);
  } catch (error) {
...
export default function PurposeOfProcessingViewPage({ params }: PageProps) {
  return (
    <div className="pt-4 px-6 h-full">
      <Suspense fallback={<div>Loading...</div>}>
        <PurposeOfProcessingViewContent params={params} />
      </Suspense>
    </div>
  );
}
